...
class SWLSTMCell(Layer):
    """Cell class for the SWLSTM layer.
    Shared Weight Long Short-Term Memory Network layer - Zhendong Zhang 2019
    https://doi.org/10.1016/j.apenergy.2019.04.047
    ...
    def __init__(self, units,
                 ...
                 implementation=1,
                 **kwargs):
        # Change 1: LSTMCell to SWLSTMCell
        super(SWLSTMCell, self).__init__(**kwargs)
        self.units = units
        self.activation = activations.get(activation)
        self.recurrent_activation = activations.get(recurrent_activation)
        self.use_bias = use_bias
    ...
    def build(self, input_shape):
        input_dim = input_shape[-1]
        # Change 2: self.units*4 to self.units*1
        self.kernel = self.add_weight(shape=(input_dim, self.units * 1),
                                      name='kernel',
                                      initializer=self.kernel_initializer,
                                      regularizer=self.kernel_regularizer,
                                      constraint=self.kernel_constraint)
        # Change 3: self.units*4 to self.units*1
        self.recurrent_kernel = self.add_weight(
        ...
        # Change 4: There are four kernels in LSTMCell, [self.kernel_i, self.kernel_f, self.kernel_c, self.kernel_o]
        #           There are only one kernel in SWLSTMCell. [self.kernel_s]
        ...
        if self.use_bias:
            ...
            self.bias_s = self.bias[:self.units]
        else:
    ...
    def call(self, inputs, states, training=None):
        ...
        if self.implementation == 1:
            if 0 < self.dropout < 1.:
                # Change 10: similar to Change 4
                inputs_s = inputs * dp_mask[0]
            else:
                # Change 11: similar to Change 4
                inputs_s = inputs
            # Change 12: similar to Change 4
            x_s = K.dot(inputs_s, self.kernel_s)
            if self.use_bias:
                # Change 13: similar to Change 4
                x_s = K.bias_add(x_s, self.bias_s)

            if 0 < self.recurrent_dropout < 1.:
                # Change 14: similar to Change 4
                h_tm1_s = h_tm1 * rec_dp_mask[0]
            else:
                # Change 15: similar to Change 4
                h_tm1_s = h_tm1
            # Change 16: core of SWLSTM, please refer to the paper: https://doi.org/10.1016/j.apenergy.2019.04.047
            s = self.recurrent_activation(x_s + K.dot(h_tm1_s,
                                                      self.recurrent_kernel_s))
            c = s * c_tm1 + (1-s) * self.activation(x_s + K.dot(h_tm1_s,
                                                            self.recurrent_kernel_s))
        else:
            if 0. < self.dropout < 1.:
                inputs *= dp_mask[0]
            ...
            if self.use_bias:
                z = K.bias_add(z, self.bias)
            # Change 17: similar to Change 4
            z0 = z[:, :self.units]
            # Change 18: core of SWLSTM, similar to Change 4
            s = self.recurrent_activation(z0)
            c = s * c_tm1 + (1-s) * self.activation(z0)

        # Change 19: core of SWLSTM, similar to Change 4
        h = s * self.activation(c)
        if 0 < self.dropout + self.recurrent_dropout:
            if training is None:
                h._uses_learning_phase = True
        return h, [h, c]
    ...
    def get_config(self):
        config = {'units': self.units,
                  ...
                  'recurrent_dropout': self.recurrent_dropout,
                  'implementation': self.implementation}
        # Change 20: LSTMCell to SWLSTMCell
        base_config = super(SWLSTMCell, self).get_config()
        return dict(list(base_config.items()) + list(config.items()))
...
class SWLSTM(RNN):
    ...
    def __init__(self, units,
        ...
        # Change 21: LSTMCell to SWLSTMCell
        cell = SWLSTMCell(units,
                        activation=activation,
                        recurrent_activation=recurrent_activation,
                        use_bias=use_bias,
                        kernel_initializer=kernel_initializer,
                        ...
                        recurrent_dropout=recurrent_dropout,
                        implementation=implementation)
        # Change 22: LSTM to SWLSTM
        super(SWLSTM, self).__init__(cell,
                                   return_sequences=return_sequences,
                                   return_state=return_state,
                                   go_backwards=go_backwards,
                                   stateful=stateful,
    ...
    def call(self, inputs, mask=None, training=None, initial_state=None):
        self.cell._dropout_mask = None
        self.cell._recurrent_dropout_mask = None
        # Change 23: LSTM to SWLSTM
        return super(SWLSTM, self).call(inputs,
                                      mask=mask,
                                      training=training,
                                      initial_state=initial_state)
    ...
    def get_config(self):
        ...
        # Change 24: LSTM to SWLSTM
        base_config = super(SWLSTM, self).get_config()
        del base_config['cell']
        return dict(list(base_config.items()) + list(config.items()))
